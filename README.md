# OP-TEE and PSA Crypto API Interoperability Prototype

## Introduction

This repository contains prototypes demonstrating how the [PSA Crypto API](https://armmbed.github.io/mbed-crypto/html/) can be integrated into the [OP-TEE](https://www.op-tee.org/) Trusted Execution Environment to improve interoperability with the [GlobalPlatform (GP) TEE Internal Core API](https://globalplatform.org/specs-library/tee-internal-core-api-specification/).
The goal of this work, as part of the [CROSSCON project](https://crosscon.eu/), is to enable Trusted Applications to use the PSA Crypto API seamlessly within OP-TEE, bridging the gap between two important industry standards for secure environments.

Two integration approaches are provided:

- **Wrapper approach** — Maps PSA Crypto API calls to OP-TEE crypto functions using an adapter layer.
- **Syscall approach** — Extends OP-TEE OS to handle PSA Crypto API calls directly through new syscalls.

This prototype serves as a reference for developers, researchers, and integrators working on TEE-based solutions that need both PSA and GP API support.

## Overview

The repository contains **repo manifests** for setting up complete OP-TEE development environments with PSA Crypto integration.
Using these manifests, one can quickly clone and build all necessary components to run the prototypes in QEMU.

High-level architecture for each approach:

- **Wrapper**:
  `TA → Wrapper Layer → OP-TEE API → OP-TEE OS → Crypto Implementation (Mbed TLS)`
- **Syscall**:
  `TA → PSA Crypto API → Syscall Interface → OP-TEE OS → Crypto Implementation (Mbed TLS)`

Both prototypes use:
- **OP-TEE**: v4.2.0
- **Mbed TLS**: v3.6.1 for PSA Crypto

## Common Setup

The steps below apply to both approaches unless otherwise noted.
The only difference will be the **manifest file** passed to `repo init`.
...
Choose the manifest based on the approach you want to build:
...
# Wrapper approach
repo init -u https://github.com/crosscon/manifest.git -m wrapper_qemu.xml

# Syscall approach
repo init -u https://github.com/crosscon/manifest.git -m psa_syscalls_qemu.xml
...
At this point, the base setup is complete.
...
> These commands correspond to the default OP-TEE examples that were modified to include PSA calls. These calls should not produce output; they perform self-validation using constant values. Errors or mismatches will trigger error messages.
...
To ensure compatibility with this approach, your Trusted Application (TA) source file must implement the following functions:
...
- TA_CreateEntryPoint
  - Called when the TA entry point is created.
- TA_OpenSessionEntryPoint
  - Called when a new session is opened.
  - For the wrapper approach this function must initialize both the wrapper and PSA Crypto. At minimum, it should include:
...
- TA_InvokeCommandEntryPoint
  - Called whenever a command is invoked by the host application.
  - This function should call the relevant PSA Crypto function or your own wrapper functions containing those calls.
